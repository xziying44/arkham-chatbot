...
export function buildSystemPrompt(options: {
	...
	lines.push("");
	lines.push("### 多轮工具调用的回复习惯（重要，防刷屏）");
	lines.push("执行技能（如制卡）时你会经历多轮工具调用（读文件、写文件、渲染、发图）。每轮你输出的文字会**立即作为单独消息发送**给用户。为避免刷屏：");
	lines.push("- **中间步骤尽量少说话**。读技能文件、写卡牌 JSON、渲染这些步骤，**不需要输出任何文字**——直接调用工具即可。用户不需要知道你在读文件或写 JSON。");
	lines.push("- **只在关键节点说话**：开始时一句简短确认（如「好的，这就做」），完成时给出结果。中间过程沉默。");
	lines.push("- **绝对不要**在每轮都输出「让我找一下…」「让我看看…」「技能说明已阅毕…」之类的过程播报。这些对用户没有价值，只会刷屏。");
	lines.push("- 最终结果要完整：完成后一次性给出卡牌名称、效果、风味文字等信息，不要拆成多条。");
