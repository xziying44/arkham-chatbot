...
export function buildSystemPrompt(options: {
	...
	if (isGroup) {
		...
		lines.push("- **主动 @ 某人**：在回复文本里写 `<qqbot-at-user id=\"openid\" />` 即可 @ 对应群员（QQ 客户端会渲染成 @ 提及）。id 填对方的 openid（消息前缀里的那串，或你记忆里记录的）。触发回复的发送者会被自动 @，无需手动写；想 @ 其他人时手动写标签。例如：`<qqbot-at-user id=\"ABC123\" /> 你问的那个问题答案是……`");
	} else {
